...
const url = process.env["DATABASE_URL"] ?? "postgresql://trader:trader@localhost:5432/trader";
const client = new pg.Client({ connectionString: url });
await client.connect();

const q = async <T>(sql: string): Promise<T> => (await client.query<T>(sql)).rows;

console.log("── A1 · closed paper trades (bar ≥30) ──");
for (const r of await q<{ chain: string; n: string; realized: string }>(
  `SELECT chain, COUNT(*) AS n, COALESCE(SUM(realized_pnl_usd),0)::text AS realized
   FROM positions WHERE status='CLOSED' GROUP BY chain`,
)) {
  console.log(`  ${r.chain.padEnd(7)} ${r.n} closed, realized $${parseFloat(r.realized).toFixed(2)}`);
}
...
for (const r of await q<{ total: string; evaluated: string; avg: string | null; win: string | null }>(
  `SELECT COUNT(*) AS total, COUNT(evaluated_at) AS evaluated,
          ROUND(AVG(outcome_return_pct)::numeric,2) AS avg,
...
for (const r of await q<{ side: string; n: string; delta: string | null; impact: string | null }>(
  `SELECT side, COUNT(*) AS n,
          ROUND(AVG(paper_price/NULLIF(quote_price,0)-1)::numeric*100,2) AS delta,
          ROUND(AVG(quote_price_impact_bps)::numeric,0) AS impact
   FROM fill_calibration GROUP BY side`,
)) {
  console.log(`  ${r.side.padEnd(4)} ${r.n} rows · paper vs real ${r.delta ?? "–"}% · avg impact ${r.impact ?? "–"}bps`);
}
const noQuote = (await q<{ n: string }>(`SELECT COUNT(*) AS n FROM fill_calibration WHERE quote_price IS NULL`))[0];
if (noQuote && noQuote.n !== "0") console.log(`  ⚠ ${noQuote.n} rows with failed quotes (see quote_error)`);
