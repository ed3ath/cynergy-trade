...
try {
  const totals = await q<{ chain: string; rows: number | string; tokens: number | string; first: Date; last: Date }>(
  ...
  const covered = await q<{ token_address: string; samples: number | string; first: Date; last: Date }>(
  ...
  for (const r of totals) {
    ...
    console.log(
      `  ${r.chain.padEnd(7)} rows ${String(r.rows).padStart(7)}  tokens ${String(r.tokens).padStart(4)}  ` +
  ...
  for (const r of covered.slice(0, 15)) {
    console.log(
      `  ${r.token_address.slice(0, 10)}…  ${String(r.samples).padStart(5)} samples  ` +
